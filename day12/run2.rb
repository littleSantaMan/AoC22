...
#start_pos = start_positions.first

def exec(start_pos, all_points)
  step = 0
...
steps = start_positions.map do |start_pos|
  all_points.each do |_, point|
    point.path = nil
  end
  exec(start_pos, all_points)
  all_points[end_pos].path ? all_points[end_pos].path.score : nil
end.compact
